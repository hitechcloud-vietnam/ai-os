...
pub async fn get_members(State(pool): State<PgPool>, Path(org_id): Path<String>) -> Json<serde_json::Value> {
...
pub async fn get_audit_logs(State(pool): State<PgPool>, Path(org_id): Path<String>) -> Json<serde_json::Value> {
