...
async fn dispatch_task(
    ...
    let task_id = format!("task_{}", Uuid::new_v4().to_string().replace('-', "")[..12].to_string());

    // Verify target agent exists and is active
    let target = sqlx::query_scalar::<_, String>(
        "SELECT status FROM a2a_agent_cards WHERE agent_id = $1"
    )
...
async fn list_tasks(
    ...
    tasks.map(Json).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn update_task_status(
    ...
    let progress = update.get("progress_pct").and_then(|v| v.as_i64()).unwrap_or(0) as i32;
...
async fn discover_agents(
    ...
    agents.map(Json).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let database_url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgres://hitechcloud:hitechcloud_dev_2026@127.0.0.1:5432/hitechcloud".to_string());
    ...
    let app = Router::new()
        ...
        .route("/a2a/v1/agents/{agent_id}", get(get_agent).delete(deactivate_agent))
        // Agent Discovery
        .route("/a2a/v1/discover", get(discover_agents))
        // Task Dispatch & Lifecycle
        .route("/a2a/v1/tasks", post(dispatch_task).get(list_tasks))
        .route("/a2a/v1/tasks/{task_id}", get(get_task).put(update_task_status))
        // SSE Task Stream
