...
pub fn required_permission(method: &str, path: &str) -> Option<&'static str> {
    match (method, path) {
        // MCP Servers
        ("GET", p) if p.starts_with("/v1/registry/mcp-servers") && !p.contains("/mcp-servers/") => {
            Some("server.read")
        }
        ...
        // Skills
        ("GET", p) if p.starts_with("/v1/registry/skills") && !p.contains("/skills/") => {
            Some("skill.read")
        }
        ...
        // Plugins
        ("GET", p) if p.starts_with("/v1/registry/plugins") && !p.contains("/plugins/") => {
            Some("plugin.read")
        }
        ...
        ("DELETE", p) if p.starts_with("/v1/registry/installations/") => {
            Some("installation.remove")
        }
