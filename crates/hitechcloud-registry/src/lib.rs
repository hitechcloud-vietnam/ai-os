...
pub fn registry_router(_pool: PgPool) -> Router<PgPool> {
    Router::new()
        .route("/mcp-servers", post(create_mcp_server).get(list_mcp_servers))
        ...
        .route("/installations", post(create_installation).get(list_installations))
        .route("/installations/{id}", get(get_installation).delete(delete_installation))
}
...
pub async fn create_mcp_server(
    ...
    .bind(&id).bind(&req.name).bind(&req.transport).bind(&req.namespace).bind(&visibility)
...
pub async fn search_skills(
    ...
    .bind(&search_term).bind(per_page)
    .fetch_all(&pool).await?;
